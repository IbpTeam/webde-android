...
public class NetworkDiscovery {
    ...
    public NetworkDiscovery(Context context, Handler handler) {
        mContext = context;
        mHandler = handler;
    }
...
//                mNsdManager.resolveService(element, mResolveListener);
                ...
                resolveServiceCB.success(element.toJSONObject());
            }
        }
    }
...
//    }
    ...
    private void addServiceInfo(ServiceInfo info) {
        ...
        if (!isExist) {
            ...
            sendNotification("onServiceFound", getAbstractServiceInfo(info).toJSONObject().toString());
        }
    }
    ...
    private void reWriteServiceInfo(ServiceInfo info) {
        ...
        resolveServiceCB.success(getAbstractServiceInfo(info).toJSONObject());
    }
    ...
    private void removeServiceInfo(ServiceInfo info) {
        ...
        if (isExist) {
            ...
            sendNotification("onServiceLost", getAbstractServiceInfo(info).toJSONObject().toString());
        }
    }
